...
SOURCES = [{"title": "BES-IPTV", "url": "http://bit.ly/kodishqipiptv", "fanart": "http://s10.postimg.org/npeunmfx5/fanart.png","thumbnail": "http://s4.postimg.org/bjbzdjkqh/icon.png"}]

# http://s10.postimg.org/npeunmfx5/fanart.png
# http://s4.postimg.org/bjbzdjkqh/icon.png
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.BES-IPTV-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(BES-IPTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(BES-IPTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR red][B]- FAVORITES CHANNEL BES-IPTV [/COLOR][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('BES-IPTV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(BES-IPTV,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from BES-IPTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to BES-IPTV Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(BES-IPTV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
